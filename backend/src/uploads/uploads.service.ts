...
/**
 * Owns the Firebase-vs-local-disk decision for where an uploaded file ends
 ...
 * automatic fallback whenever Firebase isn't configured, so `start:dev` and
 * e2e tests need zero Firebase credentials.
 */
@Injectable()
export class UploadsService {
  constructor(private readonly firebase: FirebaseStorageClientService) {}

  async store(file: Express.Multer.File, req: Request): Promise<string> {
    const filename = `${randomUUID()}${extname(file.originalname)}`;
    if (this.firebase.isConfigured()) {
      return this.firebase.upload(`uploads/${filename}`, file.buffer, file.mimetype);
    }
